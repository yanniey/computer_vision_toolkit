# training an neural net in multiple epochs
...
animals_net.setLayerSizes(np.array([3, 50, 4])) # 3 nodes in input layer, 50 nodes in hidden layer, 4 nodes in output layer
...
# convert a sample and classification into a pair of numpy arrays
def record(sample, classification):
    return (np.array([sample], np.float32),
            np.array([classification], np.float32))

# creaet fake data, 20k samples per class
RECORDS = 20000 # number of training samples
records = []
...
# generate 100 new random samples
TESTS = 100
