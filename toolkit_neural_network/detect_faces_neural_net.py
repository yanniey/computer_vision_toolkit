...
face_model = cv2.dnn.readNetFromCaffe(
    '../dataset/detect_face_data/detection/deploy.prototxt',
    '../dataset/detect_face_data/detection/res10_300x300_ssd_iter_140000.caffemodel')
face_blob_height = 300
face_average_color = (104, 177, 123)
face_confidence_threshold = 0.995

age_model = cv2.dnn.readNetFromCaffe(
    '../dataset/detect_face_data/age_gender_classification/age_net_deploy.prototxt',
    '../dataset/detect_face_data/age_gender_classification/age_net.caffemodel')
age_labels = ['0-2', '4-6', '8-12', '15-20',
              '25-32', '38-43', '48-53', '60+']

gender_model = cv2.dnn.readNetFromCaffe(
    '../dataset/detect_face_data/age_gender_classification/gender_net_deploy.prototxt',
    '../dataset/detect_face_data/age_gender_classification/gender_net.caffemodel')
gender_labels = ['male', 'female']

# define image size
age_gender_blob_size = (256, 256)
age_gender_average_image = np.load(
    '../dataset/detect_face_data/age_gender_classification/average_face.npy')


# With each frame, we begin by creating an image that is the same aspect ratio as the frame, and we feed this image to the face detection neural net
...
while success:
    ...
    for face in face_results[0, 0]:
        ...
        if face_confidence > face_confidence_threshold:
            ...
            # The face detection neural net produces rectangles that are taller than they are wide. However, the age and gender classification neural nets expect square faces. 
            # Let's widen the detected face rectangle to make it a square

            # Classify the age and gender of the face based on a square region of interest that includes the neck. 
            ...
            # scale the Region of Interest to the classifiers' image size, converting it into floating-point format, and subtracting the average face
            ...
            age_id = np.argmax(age_results) # pick the class ID with the highest confidence score
            ...
            gender_id = np.argmax(gender_results) # pick the class ID with the highest confidence score
